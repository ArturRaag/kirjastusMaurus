...
function draw() {
  ...
  MathQuill_v6rrand.position(width/asukoha_nr+60,height/asukoha_nr+285);

  sisend_vastus.position(width/asukoha_nr-0,height/asukoha_nr+262);
  ...
  beginShape();
    vertex(-250/1.5,-160/1.5);
    vertex(-20/1.5,-160/1.5);
    vertex(30/1.5,-40/1.5);
    vertex(-300/1.5,-40/1.5);
  endShape(CLOSE);
  alus_node.position(190,270);
  kylg_b_node.position(190,160);
  diagonal_2_node.position(330,250);
  pop();
  ...
  stroke(0,0,255);
  strokeWeight(3);
    vertex(-250/1.5,-40/1.5);
    vertex(-250/1.5,-160/1.5);
    korgus_node.position(140,220);
  endShape();
  pop();
...
function Ylesanne(){
  ...
  trapets_b_kylg = 5;
  if (mudeli_valik=="1"){
    while(trapets_alus<trapets_b_kylg){
      trapets_alus = round_0(Math.random()*100);
      trapets_b_kylg = round_0(Math.random()*100);
    }
    trapets_korgus = round_0(Math.random()*100);
     pindala = (((trapets_alus+trapets_b_kylg)/2)*trapets_korgus);
     perimeeter = 0
     ...
     if (suuruse_valik=="a"){
      yl_text.html("Leia trapetsi alus a, kui trapetsi pindala S on S = "+str(pindala)+", trapetsi kõrgus h on h = "+str(trapets_korgus)+" ning küljega a paralleelne külg b on b = "+str(trapets_b_kylg)+".");
      vastus=trapets_alus;
      katex.render("a = ",sisend_vastus.elt);
      alus_node.html("a = ?");
      korgus_node.html("h = "+str(trapets_korgus));
      kylg_b_node.html("b = "+str(trapets_b_kylg));
      diagonal_2_node.html("");
     } else if (suuruse_valik=="h"){
      yl_text.html("Leia trapetsi kõrgus h, kui trapetsi pindala S on S = "+str(pindala)+", trapetsi alus a on a = "+str(trapets_alus)+" ning küljega a paralleelne külg b on b = "+str(trapets_b_kylg)+".");
      vastus=trapets_korgus;
      katex.render("h = ",sisend_vastus.elt);
      alus_node.html("a = "+str(trapets_alus));
      korgus_node.html("h = ?");
      kylg_b_node.html("b = "+str(trapets_b_kylg));
      diagonal_2_node.html("");
     } else if (suuruse_valik=="b"){
      yl_text.html("Leia trapetsi alus b, kui trapetsi pindala S on S = "+str(pindala)+", trapetsi alus a on a = "+str(trapets_alus)+" ning trapetsi kõrgus h on h = "+str(trapets_korgus)+".");
      vastus=trapets_b_kylg;
      katex.render("b = ",sisend_vastus.elt);
      alus_node.html("a = "+str(trapets_alus));
      korgus_node.html("h = "+str(trapets_korgus));
      kylg_b_node.html("b = ?");
      diagonal_2_node.html("");
     } else if (suuruse_valik=="S"){
      yl_text.html("Leia trapetsi pindala S, kui trapetsi alus a on a = "+str(trapets_alus)+", trapetsi kõrgus h on h = "+str(trapets_korgus)+" ning trapetsi alusega a paralleelne külg b on b= "+str(trapets_b_kylg)+".");
      vastus=pindala;
...
function write_texts(){
  ...
  kylg_b_node=createP("c");
  kylg_b_node.position(width/asukoha_nr+155,height/asukoha_nr+65);
  kylg_b_node.style("font-family: 'Roboto',sans-serif;line-height: 140%; font-size: 1.00rem ");
...
function Reset(){
  ...
  ALISTUMIS_NUPP.position(width/asukoha_nr+200,height/asukoha_nr+257);
