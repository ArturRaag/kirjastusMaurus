...
function draw() {
  ...
  tulemus.position(width/asukoha_nr+0,height/asukoha_nr+220);
  ...
  MathQuill_v6rrand.position(width/asukoha_nr+60,height/asukoha_nr+265);
  MathQuill_v6rrand2.position(width/asukoha_nr+60,height/asukoha_nr+315);
  
  sisend_vastus.position(width/asukoha_nr-0,height/asukoha_nr+242);
  sisend_vastus2.position(width/asukoha_nr-0,height/asukoha_nr+292);
  // ----------------------------------------KOLMNURGA JOONIS
  push();
  stroke(0);
  strokeWeight(2);
  ...
  beginShape();
    vertex(-300/1.5,-170/1.5);
    vertex(-170/1.5,-170/1.5);
    vertex(-120/1.5,-50/1.5);
    vertex(-250/1.5,-50/1.5);
  endShape(CLOSE);
  alus_node.position(160,270);
  diagonal_1_node.position(200,170);
  diagonal_2_node.position(230,250);
  pop();
...
function Ylesanne(){
  ...
  if (mudeli_valik=="1"){
    romb_d1 = 50;
    romb_d2 = 1;
    while (romb_d2<romb_d1){
      romb_d1 = round_0(Math.random()*100);
      romb_d2 = round_0(Math.random()*100);
    }
    romb_alus = round_1(Math.sqrt( (romb_d1/2)*(romb_d1/2)+(romb_d2/2)*(romb_d2/2) ));

    answerSpan2 = document.getElementById('answer2')
    answerSpan2.style.display="none";

      yl_text.html("Leia rombi külg a, kui rombi esimene diagonaal d<sub>1</sub> on d<sub>1</sub> = "+str(romb_d1)+" ning teine diagonaal d<sub>2</sub> on d<sub>2</sub> = "+str(romb_d2)+".<br>Tulemus ümarda kümnendikeni.");
      vastus=romb_alus;
      katex.render("a = ",sisend_vastus.elt);
      katex.render("",sisend_vastus2.elt);
      alus_node.html("");
      korgus_node.html("");
      diagonal_1_node.html("d<sub>1</sub> = "+str(romb_d1));
      diagonal_2_node.html("d<sub>2</sub> = "+str(romb_d2));
  ...
  romb_alus = round_1(Math.sqrt( (romb_d1/2)*(romb_d1/2)+(romb_d2/2)*(romb_d2/2) ));
  perimeeter = romb_alus*4;

    yl_text.html("Leia rombi ümbermõõt P, kui rombi esimene diagonaal d<sub>1</sub> on d<sub>1</sub> = "+str(romb_d1)+" ning teine diagonaal d<sub>2</sub> on d<sub>2</sub> = "+str(romb_d2)+".<br>Tulemus ümarda kümnendikeni.");
    vastus=perimeeter;
  ...
  answerSpan2 = document.getElementById('answer2')
  answerSpan2.style.display="block";
  ...
  alfa = round_0(2 * Math.atan((poolik_d1/poolik_d2))*180/Math.PI);
  beta = round_0(2 * Math.atan((poolik_d2/poolik_d1))*180/Math.PI);
  
  console.log(alfa,beta)
    yl_text.html("Leia rombi sisenurgad &#x3B1 ja &#x3B2, kui rombi esimene diagonaal d<sub>1</sub> on d<sub>1</sub> = "+str(romb_d1)+" ning teine diagonaal d<sub>2</sub> on d<sub>2</sub> = "+str(romb_d2)+".<br>Ümarda nurgad täis kraadideks.");
    vastus=[alfa,beta];
    katex.render("\\alpha = ",sisend_vastus.elt);
    katex.render("\\beta = ",sisend_vastus2.elt);
    alus_node.html("");
    korgus_node.html("");
    diagonal_1_node.html("d<sub>1</sub> = "+str(romb_d1));
    diagonal_2_node.html("d<sub>2</sub> = "+str(romb_d2));
...
function write_texts(){
  ...
  sisend_vastus2=createP("");
  sisend_vastus2.style("font-family: 'Roboto',sans-serif;line-height: 140%; font-size: 1.25rem ");
...
function Kontroll(){
  ...
  if (mudeli_valik=="3"){
    ...
    } else if ((sisu_1==vastus[0] && sisu_2==vastus[1]) || (sisu_2==vastus[0] && sisu_1==vastus[1]) ){
      tulemus.html("Korras!");
...
function Reset(){
  ...
  ALISTUMIS_NUPP.position(width/asukoha_nr+200,height/asukoha_nr+236);
