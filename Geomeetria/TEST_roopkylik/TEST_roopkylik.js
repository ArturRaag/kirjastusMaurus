...
function draw() {
  ...
  beginShape();
    ...
    vertex(-20/1.5,-220/1.5);
    vertex(30/1.5,-100/1.5);
    vertex(-250/1.5,-100/1.5);
  endShape(CLOSE);
  alus_node.position(190,230);
  kylg_b_node.position(310,170);
  diagonal_2_node.position(330,210);
  pop();
...
function Ylesanne(){
  ...
  if (mudeli_valik=="1"){
     roopkylik_alus = round_0(Math.random()*100);
     roopkylik_korgus = round_0(Math.random()*100);
     roopkyliku_b_kylg = round_0(Math.random()*100);
     pindala = (roopkylik_alus*roopkylik_korgus);
     perimeeter = 2*(roopkylik_alus+roopkyliku_b_kylg);

     suurused=["a1","a2","h","b","S","P"];
     suuruse_valik=random(suurused);

     if (suuruse_valik=="a1"){
      yl_text.html("Leia rööpküliku alus a, kui rööpküliku pindala S on S = "+str(pindala)+" ning rööpküliku kõrgus h on h = "+str(roopkylik_korgus)+".");
      vastus=roopkylik_alus;
      katex.render("a = ",sisend_vastus.elt);
      alus_node.html("a = ?");
      korgus_node.html("h = "+str(roopkylik_korgus));
      kylg_b_node.html("b = "+str(roopkyliku_b_kylg));
      diagonal_2_node.html("");
     } else if (suuruse_valik=="h"){
      yl_text.html("Leia rööpküliku kõrgus h, kui rööpküliku pindala S on S = "+str(pindala)+" ning rööpküliku alus a on a = "+str(roopkylik_alus)+".");
      vastus=roopkylik_korgus;
      katex.render("h = ",sisend_vastus.elt);
      alus_node.html("a = "+str(roopkylik_alus));
      korgus_node.html("h = ?");
      kylg_b_node.html("b = "+str(roopkyliku_b_kylg));
      diagonal_2_node.html("");
     } else if (suuruse_valik=="b"){
      yl_text.html("Leia rööpküliku külg b, kui rööpküliku ümbermõõt P on P = "+str(perimeeter)+" ning rööpküliku alus a on a = "+str(roopkylik_alus)+".");
      vastus=roopkyliku_b_kylg;
      katex.render("b = ",sisend_vastus.elt);
      alus_node.html("a = "+str(roopkylik_alus));
      korgus_node.html("h = "+str(roopkylik_korgus));
      kylg_b_node.html("b = ?");
      diagonal_2_node.html("");
     }else if (suuruse_valik=="a2"){
      yl_text.html("Leia rööpküliku külg a, kui rööpküliku ümbermõõt P on P = "+str(perimeeter)+" ning rööpküliku teine külg b on b = "+str(roopkyliku_b_kylg)+".");
      vastus=roopkylik_alus;
      katex.render("b = ",sisend_vastus.elt);
      alus_node.html("a = "+str(roopkylik_alus));
      korgus_node.html("h = "+str(roopkylik_korgus));
      ...
      yl_text.html("Leia rööpküliku pindala S, kui rööpküliku alus a on a = "+str(roopkylik_alus)+" ning rööpküliku kõrgus h on h = "+str(roopkylik_korgus)+".");
      vastus=pindala;
      ...
      yl_text.html("Leia rööpküliku ümbermõõt P, kui rööpküliku alus a on a = "+str(roopkylik_alus)+" ning rööpküliku teine külg b on b = "+str(roopkyliku_b_kylg)+".");
      vastus=perimeeter;
