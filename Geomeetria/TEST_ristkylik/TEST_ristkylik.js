...
function draw() {
  ...
  beginShape();
    ...
    vertex(-210,-160);
  endShape(CLOSE);
  kaatet_2_node.position(285,170);
  kaatet_1_node.position(160,230);
  pop();
...
function Ylesanne(){
  ...
  korgus=round_0(random()*100);
  pindala=alus*korgus;
  perimeeter=2*(alus+korgus);
  
  if (mudeli_valik=="1"){
    yl_text.html("Leia ristküliku pindala S, kui ristküliku alus a = "+str(alus)+" ning ristküliku kõrgus b = "+str(korgus)+".");
    vastus=pindala;
    otsitav="S";
    kaatet_1_node.html("a = "+str(alus));
    kaatet_2_node.html("b = "+str(korgus));
  } else if (mudeli_valik=="2"){
    yl_text.html("Leia ristküliku pindala S, kui ristküliku üks külgedest on a = "+str(alus)+" ning ristküliku ümbermõõt on P = "+str(perimeeter)+".");
    vastus=pindala;
    otsitav="S";
    kaatet_1_node.html("a = "+str(alus));
    kaatet_2_node.html("b = ?");
  } else if (mudeli_valik=="3"){
      yl_text.html("Leia ristküliku ümbermõõt P, kui ristküliku üks külg on a = "+str(alus)+" ning ristküliku teine külg on b = "+str(korgus)+".");
      vastus=perimeeter;
      otsitav="P";
      kaatet_1_node.html("a = "+str(alus));
      kaatet_2_node.html("b = "+str(korgus));
  } else if (mudeli_valik=="4"){
      yl_text.html("Leia ristküliku ümbermõõt P, kui ristküliku pindala on S = "+str(pindala)+" ning ristküliku üks külgedest on a = "+str(alus)+".");
      vastus=perimeeter;
      otsitav="P";
      kaatet_1_node.html("a = "+str(alus));
      kaatet_2_node.html("b = ?");
  } else if (mudeli_valik=="5"){
      yl_text.html("Leia üks ristküliku külgedest a, kui ristküliku ümbermõõt P on P = "+str(perimeeter)+" ning teine külg on b = "+str(korgus)+".");
      vastus=alus;
      otsitav="a";
      kaatet_1_node.html("a = ?");
      kaatet_2_node.html("b = "+str(korgus));
  } else if (mudeli_valik=="6"){
      yl_text.html("Leia üks ristküliku külgedest a, kui ristküliku pindala S on S = "+str(pindala)+" ning teine külg on b = "+str(korgus)+".");
      vastus=alus;
      otsitav="a";
      kaatet_1_node.html("a = ?");
      kaatet_2_node.html("b = "+str(korgus));
