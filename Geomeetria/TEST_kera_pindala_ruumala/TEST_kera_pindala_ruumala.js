...
function Ylesanne(){
  ...
  if (mudeli_valik=="1"){ // KERA
     kera_raadius = round_0(Math.random()*50);
    
     kera_pindala = round_2(4* pi *kera_raadius*kera_raadius);
     kera_ruumala = round_2(4/3 *pi * kera_raadius*kera_raadius*kera_raadius);

  console.log("St: "+str(kera_pindala)+", V: "+str(kera_ruumala));
     suurused=["S","V"];
     suuruse_valik=random(suurused);
     if (suuruse_valik=="S"){
      yl_text.html("Kera raadius on R = "+str(kera_raadius)+". Leia kera täispindala St.<br>Kasuta &pi; ümardatud väärtust &pi; = 3.14.");
      vastus=kera_pindala;
      katex.render("S = ",sisend_vastus.elt);
      korgus_node.html("R = "+str(kera_raadius));
      diagonal_1_node.html("");
      diagonal_2_node.html("");
     } else if (suuruse_valik=="V"){
      yl_text.html("Kera raadius on R = "+str(kera_raadius)+". Leia kera ruumala V.<br>Kasuta &pi; ümardatud väärtust &pi; = 3.14.");
      vastus=kera_ruumala;
